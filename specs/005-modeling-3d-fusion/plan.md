...
- **F7 — Posicionamento paramétrico** _(P1; refatoração da pausa estratégica)_: troca o placement por **coordenada absoluta chutada** por **referência espacial declarativa + montagem nativa**. O LLM declara placement relativo à geometria real (ancorar/distribuir/alinhar, `@token(...)`/`@edge(...).along(f)`/`@body('X').bbox`); um **resolver determinístico no backend** (pré-pass sobre `ModelState`, reusa o probe `query_geometry`) emite **componentes + joints paramétricos** que sobrevivem a recompute (combine-DENTRO, joint-ENTRE — reconcilia o gate da dobradiça). Atrás de flag. **P1 gateia a fundação de montagem (joint/make_component, API-blind) ANTES** da camada declarativa. _Gate:_ caixa+tampa knuckle que ABRE, 100% declarativa/paramétrica, pelo loop visual. (ADR-022; `micro/fase-F7-posicionamento.md`; débito `tech-debt-posicionamento.md`.)
