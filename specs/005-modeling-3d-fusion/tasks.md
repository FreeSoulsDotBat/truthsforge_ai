...
| 3 | Edição manual (read-back/reconciliação) | [`micro/fase-3-edicao-manual.md`](./micro/fase-3-edicao-manual.md) | `[~]` | Código ✅ (T3.1–T3.6: timeline read-on-demand + reconciliação no planner + edição fluida + rollback via timeline/UI; testes verdes). **Aguardando gate do dono**: alterar à mão no Fusion → editar por chat → conferir que parte do estado real + "Desfazer última edição" |
