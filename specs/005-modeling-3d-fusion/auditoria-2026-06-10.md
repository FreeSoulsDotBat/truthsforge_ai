...
- **[média] F9: schema sempre ensina `align`/`gap_mm`, mas com `modeling_align_modes_enabled` OFF o resolver os ignora SILENCIOSAMENTE** — `backend/app/modeling/tool_schemas.py:638-653` (não flag-gated) vs `spatial_resolver.py:548-553` — com spatial ON + align OFF (estágio real de gates incrementais), um `align:'gap', gap_mm:2` emitido pelo LLM (ensinado pelo schema) vira snap concêntrico flush, sem warning. ADR-024 sanciona "flag OFF ⇒ align ignorado = zero regressão", mas isso vale p/ planos ANTIGOS; para planos novos com align explícito é mis-place silencioso — choque com o D4 do ADR-022 ("flag OFF → erro claro, nunca mis-place silencioso"). **→ CORRIGIDO (2026-06-12, pós-revisão do PR #50)**: `_expand_place_body` agora erra TIPADO (`SpatialRefError`) quando `align != 'center'` ou `gap_mm` chegam com a flag OFF, em vez de virar center silencioso; args sem align/gap seguem center bit-a-bit (zero regressão p/ planos antigos), alinhado ao D4 do ADR-022.
...
- **[média] F7 trio sempre no `PLANNER_TOOLSET` + schema completo, mesmo com `modeling_spatial_resolution_enabled` OFF** — `backend/app/modeling/tool_registry.py:603-656` (só `relate_bodies` é UNRELEASED) + `planner.py:1090` (schemas de todo o toolset) — com a flag OFF o LLM é ensinado (schema) a usar `place_body` sem o nudge F7, e todo uso falha no adapter. "Erro claro" cumpre o D4 do ADR-022, mas é um convite estrutural a planos fadados a falhar; gating de visibilidade pela flag (como `UNRELEASED_PLANNER_TOOLS`) fecharia a meia-ligação. **→ CORRIGIDO (2026-06-12, pós-revisão do PR #50)**: visibilidade ao planner agora é RUNTIME via `tool_registry.planner_toolset()` — `modeling_spatial_resolution_enabled` OFF oculta `place_body`/`align_axis`/`distribute_along` e `modeling_relation_placement_enabled` OFF oculta `relate_bodies` do enum do Structured Output (plano e corretor), dos schemas renderizados no prompt e da validação de volta; `PLANNER_TOOLSET` segue como superset estático (compat/auditoria) e a EXECUTABILIDADE (executor/policy/PATCH literal) não mudou. Testes dos dois estados em `test_tool_registry.py`/`test_planner_llm.py`.
