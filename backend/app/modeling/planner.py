...
def _build_messages(
    ...
    *,
    ...
    system_prompt = (
        ...
        "- MECANISMOS (F3): para peças funcionais, PREFIRA as tools dedicadas\n"
        "  a remontar tudo na mão:\n"
        "  • `metric_screw` (designation 'M6', length_mm) cria parafuso\n"
        "    completo com rosca MODELADA — não desenhe hélice à mão.\n"
        "  • `knuckle_hinge` (leaf_length_mm, knuckle_count, pin_diameter_mm)\n"
        "    cria uma dobradiça de nós. Produz os corpos `<name>_Leaf_A`,\n"
        "    `<name>_Leaf_B`, `<name>_Pin` — referencie ESSES nomes depois (não\n"
        "    invente outros). REGRA do `joint`: com `joint='revolute'` as abas\n"
        "    viram COMPONENTES e NÃO podem mais ser combinadas com a caixa —\n"
        "    use joint SÓ p/ peça que abre na tela. Para uma CAIXA+TAMPA\n"
        "    IMPRESSA, OMITA joint e `combine_bodies` `<name>_Leaf_A` na caixa\n"
        "    e `<name>_Leaf_B` na tampa (aí o conjunto fica printável).\n"
        "  • `thread` adiciona rosca real (is_internal=true p/ furo roscado que\n"
        "    recebe o parafuso; false p/ eixo). Para o PARAFUSO ENCAIXAR num\n"
        "    furo, rosque ambos com a MESMA designation (ex.: 'M6x1').\n"
        "  • `joint` (revolute/slider/rigid) cria a cinemática; `make_component`\n"
        "    transforma corpos em componentes antes de juntá-los.\n"
