...
**Geometria × visão (um veredito, duas percepções).** Com a auto-crítica ligada, o loop visual deixa de ser um ATUADOR paralelo (que replanejava recriando corpos — a causa de `BoxOuter (1)`/`Lid (2)`) e vira um SENSOR read-only: `assess_visual_findings` renderiza + critica e devolve `Finding(source="semantic")` que entram no MESMO `ModelVerdict` (a geometria mede o quantificável — duplicata/interferência/DOF; a visão informa o que ela não mede — proporção/orientação). Nenhuma das duas replaneja sozinha. **O replan visual (atuador) é OPT-IN** (`modeling_visual_autocorrect_enabled`, default OFF) e agora **NÃO-DESTRUTIVO**: a correção é instruída a EDITAR os corpos existentes (recebe os nomes; recriar um corpo presente é proibido) e um **guard determinístico reverte (rollback de timeline)** qualquer correção que mesmo assim recrie um corpo (variante `_fixed`/`(N)` de um corpo que já existia) — então o loop nunca persiste a explosão de duplicados, mesmo quando a visão alucina. Default OFF porque é um replan via LLM (lento) e a verificação primária é a auto-crítica geométrica; ligue p/ deixar a visão ATUAR com a rede de segurança do rollback.
